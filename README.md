...
# shif + tab : to see the parameter of some function
```
...
in machine learning no matter what type of data we are working on , could be a data frame like we saw , numpy will take the process of convert data into a series of number , numpy is implemented in c so it's so fast :

- Fast : optimizations writing in c code .
- behind scene optimization in c language
- vectorization via broadcasting (avoiding loops)
- Backbone of other python scientific packages (for example pandas)
...
- manipulationg and comparing arrays
- sorting arrays
- Use cases

To create numpy array

my_arary = np.array([1,2,3])
my_array.shape
my_array.dtype
my_array.ndim
my_array.size

### Here we will start creating some numpy arrays

to create some new arrays with initialize data
...
random_array2 = np.random.random()
random_array3 = np.random.rand(5,3)
np.random.seed(seed=0)
what seed function does its that , it can create random variables , but in a way that it can remeber them
np.unique(random_array4)
indexing by slicing : a3[:2,:2,:3]
